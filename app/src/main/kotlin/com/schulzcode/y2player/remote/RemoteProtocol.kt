...
object RemoteProtocol {
    ...
    const val DEFAULT_DEVICE_NAME = "Innioasis Y2"
    ...
    val SERVICE_UUID: UUID get() = UUID_STANDARD_SPP
    ...
    fun statusString(status: PlaybackStatus): String = when (status) {
        PlaybackStatus.PLAYING -> STATUS_PLAYING
        PlaybackStatus.PAUSED -> STATUS_PAUSED
        PlaybackStatus.PREPARING -> STATUS_PLAYING
        PlaybackStatus.IDLE, PlaybackStatus.ERROR -> STATUS_STOPPED
    }

    fun encodeHello(device: String = DEFAULT_DEVICE_NAME, protocol: Int = PROTOCOL_VERSION): String {
        val json = JSONObject()
    ...
    fun encodeState(
        ...
        val json = JSONObject()
        json.put("type", TYPE_STATE)
        json.put("title", title)
        json.put("artist", artist)
        json.put("album", album)
        json.put("status", status)
        json.put("position", positionMs.coerceAtLeast(0L))
        json.put("duration", durationMs.coerceAtLeast(0L))
        json.put("volume", volumePercent.coerceIn(0, 100))
        return json.toString()
    }

    fun encodePlayerState(
        title: String,
        artist: String,
        album: String,
        status: String,
        ...
        durationMs: Long,
        volumePercent: Int = 100
    ): String = encodeState(title, artist, album, status, positionMs, durationMs, volumePercent)

    fun decode(line: String): RemoteMessage? = parseMessage(line)

    fun encodeSnapshot(snapshot: PlaybackSnapshot, track: Track?, volumePercent: Int = 100): String {
        return encodeState(
            title = track?.title ?: "Y2 Player",
    ...
    fun encodeArtwork(base64: String): String {
        val json = JSONObject()
        json.put("type", TYPE_ARTWORK)
